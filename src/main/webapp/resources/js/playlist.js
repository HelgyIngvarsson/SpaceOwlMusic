...
$(document).ready(function () {
    ...
    $('#uploadtrack').on('click', function () {
        ...
        $.ajax({
            ...
            for (var key in data) {
                var audioname = data[key].name;
                audioplaylist = '<li style="margin:3%; width: 50%;border-radius: 10px;" class="list-group-item list-group-item-success"><span style="display: block">' + audioname + '</span><audio src=' + data[key].path + ' controls/> </li>';
                audioContaineraudiolist.append(audioplaylist);
            }
        });
    });
